...
TEST(WallSpaceConstructionTest, NonDegenerateRegularPolygon) {
    ...
    EXPECT_TRUE(wall_geometry.has_value()) << "Expected non-degenerate WallSpace for a regular polygon, but got nullopt.";
}
...
TEST(WallSpaceConstructionTest, NonDegenerateSimplePolygon) {
    ...
    // Expect the WallSpace to be non-degenerate
    // i.e., it is not nullopt
    EXPECT_TRUE(wall_geometry.has_value()) << "Expected non-degenerate WallSpace for a simple polygon, but got nullopt.";
}
...
TEST(WallSpaceConstructionTest, DegenerateStraightLine) {
    ...
    // i.e., it is nullopt
    EXPECT_FALSE(wall_geometry.has_value()) << "Expected degenerate WallSpace for a straight line, but got a valid geometry.";
}
...
TEST(WallSpaceConstructionTest, DegenerateSinglePoint) {
    ...
    // Expect the WallSpace to be degenerate
    // i.e., it is nullopt
    EXPECT_FALSE(wall_geometry.has_value()) << "Expected degenerate WallSpace for a single point, but got a valid geometry.";
}
...
TEST(WallSpaceConstructionTest, DegenerateRepeatedPoint) {
    ...
    // Expect the WallSpace to be degenerate
    // i.e., it is nullopt
    EXPECT_FALSE(wall_geometry.has_value()) << "Expected degenerate WallSpace for a repeated point, but got a valid geometry.";
}
...
TEST(WallSpaceConstructionTest, DegenerateSelfIntersectingPolygon) {
    ...
    // Expect the WallSpace to be degenerate
    // i.e., it is nullopt
    EXPECT_FALSE(wall_geometry.has_value()) << "Expected degenerate WallSpace for a self-intersecting polygon, but got a valid geometry.";
}
...
TEST(WallSpaceConstructionTest, DegenerateOutOfOrderPoints) {
    ...
    // Expect the WallSpace to be degenerate
    // i.e., it is nullopt
    EXPECT_FALSE(wall_geometry.has_value()) << "Expected degenerate WallSpace for a normally valid polygon with out-of-order points, but got a valid geometry.";
}
